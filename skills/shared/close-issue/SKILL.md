...
Lane retirement must be performed by a controller or operator outside the lane. Do not run `lane close` from the author or reviewer session: it removes those sessions together with the managed worktree and local branch. The external controller retires it with `tars lane close --issue <number>` (or the explicit worktree path).
